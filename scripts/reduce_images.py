...
def load_reduce_save_img(path, max_img_size, in_dir, out_dir):
    img = cv2.imread(str(path))
    img = longest_max_size(img, max_size=max_img_size, interpolation=cv2.INTER_LINEAR)
    out_path = path.replace(in_dir, out_dir)
...
if __name__ == '__main__':
    ...
    paths = [str(p) for p in in_dir.rglob('*')]

    with ProcessPoolExecutor() as p:
        p.map(lambda p: load_reduce_save_img(p, max_img_size, in_dir, out_dir), paths)
